...
export function Input({ label, name, required, readOnly, error, inputSize, children, ...props }: InputProps) {
  ...
  return (
    <div className={`h-16 flex flex-col ${readOnly ? 'text-[var(--blue-300)]' : 'text-[var(--blue-800)]'} ${inputClass.size[inputSize]} `}>
      <label htmlFor={name} className='flex gap-1 px-1 items-center text-xs w-72 text-[var(--blue-800)]'>
        {label}
      </label>
